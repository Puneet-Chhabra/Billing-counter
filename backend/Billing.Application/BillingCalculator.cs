...
public sealed record BillingLine(int MenuItemId, string ItemName, decimal UnitPrice, int Quantity, decimal Total);
...
{
    public BillingResult Calculate(IEnumerable<(MenuItem Item, int Quantity)> selections, decimal discount, bool taxEnabled, decimal gstPercentage)
    {
        var lines = selections.Select(selection =>
        {
            ...
            return new BillingLine(selection.Item.Id, selection.Item.Name, selection.Item.Price, selection.Quantity, total);
        }).ToList();
        ...
        if (gstPercentage is < 0 or > 100) throw new ArgumentOutOfRangeException(nameof(gstPercentage));
        var tax = taxEnabled ? decimal.Round((subtotal - safeDiscount) * gstPercentage / 100, 2) : 0;
